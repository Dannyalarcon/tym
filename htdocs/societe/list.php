...
while ($i < min($num, $limit))
{
	...
	if (!empty($arrayfields['s.phone']['checked']))
	{
		//comunicarse con whatsapp
		print "<td><a href='tel:$obj->phone'>".dol_print_phone($obj->phone, $obj->country_code, 0, $obj->rowid, 'AC_TEL')."</a></td>\n";
		if (!$i) $totalarray['nbfield']++;
	}
	if (!empty($arrayfields['s.phone']['checked']))
	{
		print " <td><a href='https://api.whatsapp.com/send?phone=+502$obj->phone '><button type='button' class='boto btn btn-success'><i class='fab fa-whatsapp '></i></button></a></td>\n";
		if (!$i) $totalarray['nbfield']++;
	}
	if (!empty($arrayfields['s.fax']['checked']))
	{
